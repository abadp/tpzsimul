...
//************************************************************************
   ...
   class TPZFifoMemory : public TPZRunnableComponent
   {
      typedef TPZRunnableComponent Inhereited;
      friend class TPZRouterBuilder;

   public:   
      typedef enum { NIL, WH, DAMQ, CT, CT_MCAST, WH_MCAST } TPZBufferControl;
               
      TPZFifoMemory( const TPZComponentId& id, 
                     unsigned size,
                     TPZBufferControl control );
                     
      virtual ~TPZFifoMemory();
      ...
      DEFINE_RTTI(TPZFifoMemory);
   
   protected:      
      virtual void buildFlowControl();

   private:
      static TPZFifoMemory* newFrom(const TPZTag* tag, TPZComponent* owner);
