...
//*************************************************************************
//:
//    File: TPZCrossbarFlowVCMuxOpt.cpp
//
//    Class:  TPZCrossbarFlowVCMuxOpt
//
//    Inherited from:  TPZCrossbarFlowVCMux
// :
//*************************************************************************
//end of header
...
IMPLEMENT_RTTI_DERIVED(TPZCrossbarFlowVCMuxOpt,TPZCrossbarFlowVCMux);

//*************************************************************************
//:
//  f: TPZCrossbarFlowVCMuxOpt ();
//
//  d:
//:
//*************************************************************************

TPZCrossbarFlowVCMuxOpt :: TPZCrossbarFlowVCMuxOpt( TPZComponent& component)
                    : TPZCrossbarFlowVCMux(component)
...
//:
//*************************************************************************
Boolean TPZCrossbarFlowVCMuxOpt::checkPipelineForMessageAt(unsigned iPort) 
{
   TPZMessage* msg;
   ...
   //first we check if the output port is free
   unsigned portout = extractOutputPortNumber(msg);
   if (getStateForOutputPort(portout)!=FREE) return false;
   
   //then we check if input multiplexer is also free (Mux Crossbar)
   unsigned portin = getFisicalIportFor(iPort);
   if (getStateForInputPort(portin)!=FREE) return false;
   ...
   setStateForInputPort(portin, ASIGNED);
